...
public class CourseServiceImpl implements CourseService {
    ...
    public Course create(Course mk) {
        List<Lecturer> processedLecturers = new ArrayList<>();

        for (Lecturer lecturer : mk.getDosenPengampu()) {
            Lecturer existingLecturer = lecturerRepository.findByNama(lecturer.getNama())
                    .orElseGet(() -> lecturerRepository.save(lecturer));
            processedLecturers.add(existingLecturer);
        }

        mk.setDosenPengampu(processedLecturers);
